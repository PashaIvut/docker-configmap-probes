...
CrashLoopBackOff — контейнер постоянно падает и перезапускается. Причина: неправильная настройка лимитов. Быстрая диагностика: kubectl get pods показывает RESTARTS растущим, статус CrashLoopBackOff.
